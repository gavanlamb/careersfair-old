define(["jquery", "underscore", "backbone", "text!templates/app/tab-nav-linkedin.html", "views/select-group", "helper/pubsub"],
function($, _, Backbone, _tabNavTemplate, SelectGroup, PubSub) {
    return Backbone.View.extend({
        ...
        render: function() {
            this.$el.append("<div class='actionsheadingrow col-xs-12 col-sm-12 col-md-12 col-lg-12'><div class='col-xs-6 col-sm-6 col-md-5 col-lg-4 col-md-offset-1 col-lg-offset-1'><h2 class='actionsheading'>Form Fields</h2></div><div class='col-xs-6 col-sm-6 col-md-5 col-lg-4 col-md-offset-1 col-lg-offset-1'><h2 class='actionsheading'>Linkedin Fields</h2></div></div>");
        ...
        changeSelected: function() {
            for (var i = 0; i < this.selectGroupList.length; i++) {
                ...
                if (this.selectGroupList.length > 1 && ($(this.selectGroupList[i].selectForm.el).val() == "" && $(this.selectGroupList[i].selectLinkedInList[0].el).val() == "") ||
                ((this.selectGroupList[0].selectForm.collection.length < this.selectGroupList.length  && this.selectGroupList.length > 1) && $(this.selectGroupList[i].selectForm.el).val() == "" && $(this.selectGroupList[i].selectLinkedInList[0].el).val() != "") ) {
            ...
            if ($(this.selectGroupList[this.selectGroupList.length - 1].selectForm.el).val() != "" && $(this.selectGroupList[this.selectGroupList.length - 1].selectLinkedInList[selectLinkedInListIndex].el).val() != "" && this.selectGroupList.length < this.selectGroupList[0].selectForm.options.collection.length) {
                this.selectGroupList.push(new SelectGroup({
                    id: this.idSelectGroup,
                    collection: this.collection
                }));
